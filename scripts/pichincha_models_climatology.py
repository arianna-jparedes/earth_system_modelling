...
for var, meta in variables.items():
    ...
    ax.plot(range(1, 13), era5_vals, color="black", linewidth=3,
            linestyle="-", marker="o", markersize=6, label="ERA5", zorder=5)

    # Models
    for (ds, label, color) in models:
        vals = ds[var].values.squeeze()
        ax.plot(range(1, 13), vals, linewidth=2.5, linestyle="--",
                marker="s", markersize=5, color=color, label=label)

    ax.set_title(meta["title"], fontsize=20, fontweight="bold", pad=12)
    ax.set_ylabel(meta["ylabel"], fontsize=18, fontweight="bold")
...
for var, meta in variables.items():
    ...
    ax.set_title(meta["title2"], fontsize=20, fontweight="bold", pad=12)
    ax.set_ylabel(meta["ylabel"], fontsize=18, fontweight="bold")
    ax.set_xlabel("Month", fontsize=18, fontweight="bold")
    ax.set_xticks(range(1, 13))
    ax.set_xticklabels(months, fontsize=18)
