...
x = np.arange(12)
fig, ax1 = plt.subplots(figsize=(9,8), dpi=200)
...
ax1.set_xlabel("Months", fontweight="bold", fontsize=14)
ax1.set_ylabel("mm day$^{-1}$", fontweight="bold", fontsize=14)

# Right y axis
ax2 = ax1.twinx()
ax2.plot(x, t.values, color="darkred", linewidth=2)
ax2.set_ylabel("Temperature (°C)", fontweight="bold", fontsize=14, color="darkred")
...
# Inline labels
ax1.text(x[8] + 0.3, pev.values[8] + 0.3, "PET", color="black", va="center", fontsize=14)
ax1.text(x[1] + 0.3, tp.values[1] + 0.3, "P", color="royalblue", va="center", fontsize=14)
ax1.text(x[3] + 0.3, e.values[3] + 0.3, "E", color="orange", va="center", fontsize=14)
ax2.text(x[6] + 0.3, t.values[6] + 0.1, "T2M", color="darkred", va="center", fontsize=14)
plt.title("Water Balance from ERA5 (Pichincha-Ecuador)", fontsize=16, fontweight="bold", pad=15)
